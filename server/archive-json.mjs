// Archivio prezzi su file JSON: la riserva per quando SQLite non c'è.
//
...
export function creaArchivioJson(percorsoFile) {
const FILE = resolve(percorsoFile)
let dati = carica()
...
function ricordaGiocatore(player) {
  if (!player?.id) return
 ...
 */
function registraPrezzo(id, platform, quote, origine = 'sorgente', istante = Date.now()) {
  ...
  dati.storico[chiave][platform] = recordSnapshot(dati.storico[chiave][platform] ?? [], price, istante)
  potaSeTroppoGrande()
  programmaSalvataggio()
}

function leggiPrezzo(id, platform) {
...
function statistiche() {
  ...
  return {
    ...
    carte,
    puntiStorico: punti,
    interesse: dati.interesse.length,
...
function salvaOra() {
  ...
  return {
    ...
    statistiche,
    salvaOra,
    /** Senza database questa domanda non si può fare: si risponde vuoto. */
    movimenti() {
      return []
    },
    pota() {
