...
export function parsePriceList(text: string): { name: string; price: number }[]
export function pickBestMatch<T extends { name: string; rating?: number }>(name: string, players: T[]): T | null
export function matchKnownPlayer<T extends { name: string; rating?: number }>(name: string, players: T[]): T | null
